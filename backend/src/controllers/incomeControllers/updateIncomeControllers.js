...
export const updateIncome = async (request, response) => {
  try {
    ...
    const { incomeName, incomeCategory, amount, expectedPayOut, frequency } =
      request.body;
    ...
    if (!incomeCategories.includes(incomeCategory)) {
      return response.status(400).json({ message: "Invalid income category" });
    }

    if (!incomeFrequencies.includes(frequency)) {
      return response.status(400).json({ message: "Invalid income frequency" });
    }
